// The authoritative game engine: pure, environment-agnostic simulation. No
// localStorage, no DOM, no React — only game state in, game state out. This runs
// unchanged in the browser today and in a Node/server authority tomorrow. All
// randomness goes through rng() (see ./rng) so the server can own it.
import {
  AttackStyle, EquipSlot, GameState, LogKind, Monster, Settings, SkillId, StatKey, WorldState,
} from './types';
import { ITEMS } from './items';
import { GATHER_MAP, THIEVE_MAP } from './actions';
...
import { SLAYER_MASTER_MAP, SLAYER_REWARDS } from './slayer';
import { QUEST_MAP } from './quests';
import { ACHIEVEMENTS } from './achievements';
import { HOME, RESOURCE_BY_CHAR, SPAWNS, STATIONS, WORLD_VERSION, isWalkable, tileAt } from './world';
import { rng } from './rng';
...
function monsterAttacks(s: GameState, m: Monster) {
  if (rng() < monsterHitChance(s, m)) {
    const dmg = reduceIncoming(s, 1 + Math.floor(rng() * m.maxHit));
    s.currentHp -= dmg;
...
function playerAttacks(s: GameState, m: Monster): boolean {
  ...
  let dmg = 0;
  if (rng() < playerHitChance(s, m)) {
    dmg = 1 + Math.floor(rng() * playerMaxHit(s));
...
function doThieveTick(s: GameState, targetId: string) {
  ...
  if (rng() < thieveChance(lvl(s, 'thieving'), t.level)) {
...
export function withAchievements(s: GameState): GameState {
...
export function simulateOffline(s: GameState, ticks: number) {
  const before = { ...s.xp };
  ...
  switch (action.type) {
    case 'TICK': {
      ...
      if (s.activity?.type === 'gather') {
        ...
        if (rng() < gatherChance(lvl(s, a.skill), a.level)) {
          addItem(s, a.output, 1);
          s.stats.itemsGathered += 1;
          grantXp(s, a.skill, a.xp);
          if (a.skill === 'mining' && rng() < 0.01) {
    ...
    case 'NEW_TASK': {
      ...
      const monsterId = pool[Math.floor(rng() * pool.length)];
